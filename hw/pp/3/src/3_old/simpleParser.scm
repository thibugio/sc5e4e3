  ...
  (lambda ()
    (let ((nextsymbol (car (get-next-symbol))))
      (cond
        ...
        ((eq? nextsymbol 'COMMENT) (comment-parse))
        (else (begin
                (unget-next-symbol)
...
; parse a comment line. continue until another comment symbol is reached

(define comment-parse
  (lambda ()
    (if (or (eq? (car (get-next-symbol)) 'COMMENT) (eq? (car (get-next-symbol)) 'EOF))
        '()
        (begin
          (unget-next-symbol)
          (comment-parse)))))
...
; strip empty lists from a list of lists

(define strip-empty
  (lambda (l)
    (cond
      ((null? l) l) ; should never happen...
      ((null? (car l)) (strip-empty (cdr l)))
      (else (cons (car l) (strip-empty (cdr l)))))))
